...
Diferença em relação à Fase 2 (dxf_scale): aquela é uma escala "genérica"
(qualquer DXF). Esta fase é específica para o fluxo de nesting do FORAN —
exige um bloco mestre (default "PartBlock") e camadas com nomes
convencionados ("Parts", "Labels"). Os dois módulos são independentes.
"""
...
def _escalar_e_cotar_single_file(path: Path, params: dict, out_dir: Path, log):
    ...
    # 0. Garante a layer de cotas e limpa execuções anteriores
    #    (idempotência: excluir ANTES de cotar de novo)
    garantir_layer(doc, layer_cota, cor=3)
    excluir_layer_completa(doc, layer_cota)
    if layer_textos_antigos:
        excluir_layer_completa(doc, layer_textos_antigos)

    # Estilo de cota dimensionado para a escala alvo (sem casas decimais)
    estilo_cota_global = {
        "dimtxt": 2.5 * escala,
        "dimasz": 1.5 * escala,
        "dimexe": 1.0 * escala,
    ...
    # 4. Duplica/desloca labels
    if layer_textos_antigos or True:
        # a layer de labels é fixa em "Labels" — ver duplicar_e_deslocar_labels
...
def escalar_e_cotar(files: list, params: dict, dest_dir):
    ...
    Args:
        ...
        params: dict com as chaves
            ...
            layer_textos_antigos (default "Free Texts", pode ser "" p/ pular limpeza)
